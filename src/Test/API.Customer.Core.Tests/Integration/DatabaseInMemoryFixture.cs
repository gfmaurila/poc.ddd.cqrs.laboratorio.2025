...
using API.Customer.Core.Tests.Integration.Utilities.Auth;
...
public class DatabaseSQLServerFixture : IAsyncLifetime
{
    ...
    public DatabaseSQLServerFixture()
    {
        ...
        var options = new DbContextOptionsBuilder<CustomerAppDbContext>()
            .UseInMemoryDatabase("InMemoryDbForTesting_" + random.Next())
            .Options;

        _context = new CustomerAppDbContext(options);
    }
